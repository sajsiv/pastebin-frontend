  ...
  const requestUrl = `${baseUrl}`;
    ...
    useEffect(() => {
      ...
      const postComment = async(postCommentData:PasteComment) => {
        await axios.post(requestUrl + props.content.id + "/comments",postCommentData)
        window.location.href = frontendURL+props.content.id
      }

      const deleteComment = async(commentid:number) => {
          await axios.delete(requestUrl + props.content.id + "/comments/" + commentid )
          window.location.href = frontendURL+props.content.id
      }
    return (
        <>
        ...
        <button onClick={()=>post.comment!==""?postComment(post):window.alert('empty comment not allowed')}>post comment</button>
        {comments?.map(x=><li key={x.commentid}>{x.comment} <button onClick={()=>deleteComment(x.commentid)}>delete</button></li>)}
        </>
    )
